/*
 * @Date: 2023-02-13 17:08:04
 * @LastEditors: init0xyz laiyilong0@gmail.com
 * @LastEditTime: 2023-02-13 21:32:28
 * @FilePath: /gridea-neo/src/main/server.ts
 */
      ...
      .on('error', (err: NodeJS.ErrnoException) => {
        if (err) {
          if (err.message === 'getaddrinfo ENOTFOUND localhost') {
            ...
            console.log(
              '\x1B[31m%s\x1B[0m',
              'Localhost is not found so that the preview server is not working. Please check your hosts file and then restart the application.'
            )
          } else {
