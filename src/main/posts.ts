...
export default class Posts extends Model {
  ...
  public async savePosts() {
    ...
    await Promise.all(
      results.map(async (result: string, index: number) => {
    ...
    fixedResults.forEach((result: string, index: number) => {
      const postMatter = matter(result)
      const data = postMatter.data as any
      ...
      delete (postMatter as Partial<matter.GrayMatterFile<string>>).orig // Remove orig <Buffer>
      const post = {
        ...postMatter,
        abstract: '',
        fileName: ''
