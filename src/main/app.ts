...
import type { IApplicationDb, IApplicationSetting } from './interfaces/application'
import initDeployEvents from './events/deploy'
import initMenuEvents from './events/menu'
import initPostEvents from './events/post'
import initRendererEvents from './events/renderer'
import initSettingEvents from './events/setting'
import initSiteEvents from './events/site'
...
export default class App {
  ...
  public async saveSourceFolderSetting(sourceFolderPath = '') {
    try {
  ...
  private async checkDir() {
    ...
    try {
      ...
      if (fse.pathExistsSync(this.appDir)) {
        ...
        ;['images', 'config', 'post-images', 'posts', 'themes', 'static'].forEach(
          (folder: string) => {
  ...
  private checkTheme(themeName: string): void {
    ...
    if (!fse.pathExistsSync(themePath)) {
      fse.copySync(path.join(__static, 'default-files', 'themes', themeName), themePath)
    }
  }
  ...
  private updateStaticServer(): void {
    ...
    const routers = this.previewServer._router
    if (routers) {
  ...
  private initEvents(): void {
    initSiteEvents(this)
    initDeployEvents(this)
    initPostEvents(this)
    initTagEvents(this)
    initMenuEvents(this)
    initThemeSettings(this)
    initRendererEvents(this)
    initSettingEvents(this)
  }
}
