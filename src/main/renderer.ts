...
import { DEFAULT_ARCHIVES_PAGE_SIZE, DEFAULT_POST_PAGE_SIZE } from './helpers/constants'
import Model from './model'
import type { IPostDb, IPostRenderData, ISiteTagsData, ITagRenderData } from './interfaces/post'
...
export default class Renderer extends Model {
      ...
      .map((item: IPostDb) => {
        ...
        const result: IPostRenderData = {
          ...
          abstract: markdown.render(
            helper.changeImageUrlLocalToDomain(item.abstract, themeConfig.domain)
          ),
          ...
          tags: this.db.tags
            ...
            .map((tag: ITag) => ({
              ...tag,
              link: urlJoin(themeConfig.domain, themeConfig.tagPath, `${tag.slug}`, '/')
            })),
          date: item.data.date,
          dateFormat:
            (themeConfig.dateFormat && moment(item.data.date).format(themeConfig.dateFormat)) ||
            item.data.date,
          feature:
  ...
  public async renderPostList(archivePath: string) {
    ...
    for (let i = 0; i * pageSize < excludeHidePostsData.length; i += 1) {
      ...
      if (i === 0 && excludeHidePostsData.length > pageSize) {
        ...
        renderData.pagination.prev =
          i === 1
            ? urlJoin(domain, archivePath, '/')
            : urlJoin(domain, archivePath, 'page', `${i}/`)

        renderData.pagination.next =
          (i + 1) * pageSize < excludeHidePostsData.length
    ...
    for (const usedTag of usedTags) {
      ...
      for (let i = 0; i * pageSize < posts.length; i += 1) {
        ...
        if (i === 0 && posts.length > pageSize) {
          ...
          renderData.pagination.prev =
            i === 1 ? tagDomainPath : urlJoin(tagDomainPath, 'page', `${i}/`)

          renderData.pagination.next =
            (i + 1) * pageSize < posts.length ? urlJoin(tagDomainPath, 'page', `${i + 2}/`) : ''
        }
  ...
  async buildCss() {
    ...
    return new Promise((resolve, reject) => {
      less.render(
        ...
        async (err: any, output: Less.RenderOutput | undefined) => {
          if (err) {
            console.log(err)
            reject(err)
          }
  ...
  async buildFeed() {
    ...
    const feedFullText =
      typeof themeConfig.feedFullText === 'undefined' ? true : themeConfig.feedFullText
