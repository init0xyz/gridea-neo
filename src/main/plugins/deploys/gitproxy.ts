 ...
 * @LastEditTime: 2023-02-12 20:56:01
 * @FilePath: /gridea-neo/src/main/plugins/deploys/gitproxy.ts
 */
import { HttpProxyAgent, HttpsProxyAgent } from 'hpagent'
import get from 'simple-get'
import type { IApplicationDb } from '../../interfaces/application'
...
export default class GitProxy {
  ...
  public async request({
    ...
    const proxy = url.startsWith('https:') ? { Agent: HttpsProxyAgent } : { Agent: HttpProxyAgent }
    const agent =
      setting.enabledProxy === 'proxy'
        ? new proxy.Agent({
