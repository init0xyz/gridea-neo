 ...
 * @LastEditTime: 2023-02-13 17:22:43
...
export default function initDeployEvents(appInstance: any) {
  ...
  ipcMain.on('site-publish', async (event: IpcMainEvent) => {
  ...
  ipcMain.on('remote-detect', async (event: IpcMainEvent) => {
    const client = (
      {
        github: deploy,
        coding: deploy,
