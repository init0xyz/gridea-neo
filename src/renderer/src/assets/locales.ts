const message = {
  zhHans: {
    ...
    renderError:
      '渲染失败，请检查 hosts 文件中 127.0.0.1 是否指向 localhost。确认配置正确后，尝试重启应用。',
  ...
  zh_TW: {
    ...
    renderSuccess: '渲染完毕，快去预览吧！',
    renderError:
      '渲染失敗，請檢查 hosts 文件中 127.0.0.1 是否指向 localhost。確認配置正確後，嘗試重啟應用。',
  ...
  ja_JP: {
    ...
    renderError:
      'レンダリングに失敗しました。hostsファイルの127.0.0.1がlocalhostを指しているかどうかを確認してください。構成が正しいことを確認した後、アプリケーションを再起動してみてください。',
